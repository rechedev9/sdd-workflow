...
2. **During implementation**, consult SKILL.md first but use internet search freely
   when needed — trust your judgment
3. **After discoveries** (from search or implementation), update the SKILL.md so it
   stays complete over time

This protocol is inspired by [antirez's clean room methodology](https://antirez.com/latest/0):
curate documentation as a prerequisite, not a supplement. Over time, each SKILL.md converges
toward completeness as discoveries are fed back into it.
